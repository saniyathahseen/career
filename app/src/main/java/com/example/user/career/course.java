...
public class course extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ExpandableListView list1 = (ExpandableListView) findViewById(R.id.list2);
        dbhelper db=new dbhelper(this);
        Intent intent = getIntent();
        String idi = intent.getStringExtra("s");
        listDataHeader = db.cCategory(idi);
        listDataChild =prepareListData(listDataHeader,idi);
        ...
        list1.setOnChildClickListener(new ExpandableListView.OnChildClickListener() {
            ...
            public boolean onChildClick(ExpandableListView parent, View v, int groupPosition, int childPosition, long id) {
                Intent ints = new Intent(getApplicationContext(), course3.class);
                dbhelper db=new dbhelper(getApplicationContext());

                int rowid = db.getCourseId(listDataChild.get(listDataHeader.get(groupPosition)).get(childPosition));
                ints.putExtra("cid", String.valueOf(rowid));
                startActivity(ints);
                return false;
            }
        });
    ...
    private  HashMap<String, List<String>>  prepareListData(List<String> head,String idi) {
